...
WP = if (is.na(args[3])) TRUE else as.logical(args[3]) ### if TRUE then tune Win Probability models, else tune Expected Points models, default is EP
...
if (!CATALYTIC) { # CATALYTIC = FALSE
  if (!WP) { ### EP model tuning
    ...
    model_names_list <- list(
      xgb_wp_110_7_model_name, xgb_wp_1_model_name, xgb_wp_Baldwin_1_model_name
    )
  }
} else { # CATALYTIC = TRUE
  if (!WP) { ### expected points model tuning
    model_names_list <- list(
      xgb_C_110_oqdq_1_model_name, xgb_C_110_oq2xdq2x_1_model_name,
      xgb_C_oqdq_1_model_name, xgb_C_oq2xdq2x_1_model_name
      #xgb_C_nflFastR_oqdq_1_model_name
    )
    ### make sure the indices of the below list match the indices of the above list...
    catalytic_model_names_list <- list(
      "mlr_oqdq_110_2", "mlr_oq2xdq2x_110_3c",
      "mlr_yurko_oqdq_2", "mlr_yurko_oq2xdq2x_1"
    )
